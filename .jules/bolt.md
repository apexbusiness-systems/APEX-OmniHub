...
## YYYY-MM-DD - O(N) array allocation inside recursive object traversal is a major bottleneck
**Learning:** Calling `Object.keys(obj).forEach(...)` inside a recursive object traversal function allocates a new array and invokes a callback at every level of depth. This causes unnecessary garbage collection overhead and is significantly slower than a simple `for...in` loop.
**Action:** Use a `for (const k in obj)` loop with an `Object.prototype.hasOwnProperty.call(obj, k)` check. This prevents array allocations and callback overhead during recursive object traversal, resulting in a >3x speedup.
