...
## 2026-07-27 - Replaced O(N*M) list iteration with cached O(N) RegExp for high-frequency string matching
**Learning:** Using `list.some(c => body.includes(c))` inside loops over large payloads creates severe O(N*M) bottlenecks. However, blindly replacing it with dynamic RegExp compilation inside the loop moves the bottleneck to memory allocation and regex parsing, resulting in worse performance for high-throughput validators.
**Action:** Always pre-compile and cache RegExp objects at the instance level (e.g., using a Map or class property) to achieve true O(N) evaluation speed without recompilation overhead.
