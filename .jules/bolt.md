...
## YYYY-MM-DD - Replaced Object.entries() and Object.fromEntries() with for...in
**Learning:** `Object.fromEntries(Object.entries(raw).filter(...))` and iterating over `Object.entries(obj)` creates unnecessary intermediate arrays and causes noticeable overhead when dealing with high iteration counts or large payloads.
**Action:** Always replace `Object.entries()` loops with `for (const key in obj) { if (Object.prototype.hasOwnProperty.call(obj, key)) { ... } }` and avoid `Object.fromEntries()` when transforming objects. It significantly improves throughput by avoiding array allocations and garbage collection overhead.
