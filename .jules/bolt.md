...
## 2026-07-26 - Hoisting Static Arrays Outside Components
**Learning:** Using `useMemo` with an empty dependency array (`[]`) to memoize static data still incurs a small runtime overhead for React to evaluate the hook. It's better to avoid recreating the array altogether.
**Action:** Always hoist static arrays or objects completely outside of the component definition as module-level constants. This guarantees they are evaluated only once at module load time.
