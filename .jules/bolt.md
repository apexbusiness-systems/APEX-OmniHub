...
## 2026-09-04 - O(N) array search inside stringified payload is a major bottleneck\n**Learning:** Checking for substrings across an array of terms inside high-frequency policy checks (like `allow.some(c => body.includes(c))`) is extremely slow for large JSON payloads.\n**Action:** Use a pre-compiled Regular Expression cached via WeakMap (`new RegExp(list.join('|'), 'i')`) to evaluate the entire list in O(1) time against the body, replacing the O(N) array iteration.
