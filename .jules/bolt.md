...
## 2026-07-28 - Prevent unnecessary memory allocations in React Components
**Learning:** Found multiple instances where static arrays or config objects are defined within the render cycle (e.g. `AppSidebar.tsx`, `ConnectorKit.tsx`), causing them to be re-allocated on every render. Also found arrays being created just to join strings for class names (`AppTile.tsx`). While fast, these create unnecessary garbage collection overhead in a large SPA.
**Action:** Always move static definitions outside components. Use `useMemo` for objects that depend on props/state but rarely change. Use string interpolation instead of `['a', 'b'].join(' ')` for classname concatenation to minimize array allocations.
