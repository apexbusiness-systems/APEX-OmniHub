...
## 2026-08-15 - Replaced O(N) string check with O(1) RegExp lookup
**Learning:** Checking for substrings across an array of terms inside high-frequency verification functions like `isPublicPath` using `PUBLIC_PATHS.some(path => normalized.includes(path))` causes constant array iterations and unnecessary allocations when `toLowerCase` is called on every incoming path.
**Action:** Replaced `.some()` array iteration with a pre-compiled `RegExp(..., 'i')` constructed with an `escapeRegExp` helper for O(1) matching on the hot path.
