...
## 2026-07-20 - String chunking conversion memory vs cpu trade-offs
**Learning:** When converting large `Uint8Array` binary streams into strings (e.g. for `btoa` encoding) via `String.fromCodePoint(...chunk)` it causes performance issues. While chunking prevents call stack limits, using the spread operator (`...`) or `Array.from()` adds immense overhead compared to `Function.prototype.apply()`.
**Action:** Use `String.fromCharCode.apply(null, chunk as unknown as number[])` which handles ~32KB chunks 2-3x faster without exploding the stack size limits.
