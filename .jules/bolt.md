...
## 2026-08-30 - Moving Static Data Outside Component Scope
**Learning:** Initializing static arrays (like `navItems`) or objects inside a React component body forces re-creation (and memory reallocation) of that data on every render cycle. While `useMemo` is one way to avoid this, it still carries hook overhead. If the data has no dependencies on component state or props, it belongs in the module scope.
**Action:** Always hoist static arrays, configuration objects, and dictionaries outside the component function definition to eliminate unnecessary garbage collection overhead and render cost.
