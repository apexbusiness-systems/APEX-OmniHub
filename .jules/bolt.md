...
## 2026-10-27 - Replaced O(N) Array Searches with O(1) Pre-compiled Regexes in Web3 Guardrails
**Learning:** Checking path access restrictions using `PUBLIC_PATHS.some((p) => path.includes(p))` or `.startsWith()` runs an O(N) loop on every single request in middleware/guardrails.
**Action:** Always pre-compile array lookups into a single RegExp outside the function scope. Use `escapeRegExp` and join with `|` to create an O(1) lookup that is nearly 10x faster for routing.
