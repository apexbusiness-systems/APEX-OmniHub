...
## 2026-07-28 - Avoid O(N) array allocation when converting PCM buffers
**Learning:** Manual bit-shifting loops to convert a typed array (e.g. `Uint8Array` to `Int16Array`) are extremely slow and unnecessary when the underlying bytes are already in the target format (little-endian PCM). Creating intermediate TypedArrays and iterating over every byte wastes CPU cycles and GC bandwidth.
**Action:** Always use the underlying buffer directly or copy the `Uint8Array` directly when the binary representation matches the target format, making the operation O(1) in terms of data transformation.
