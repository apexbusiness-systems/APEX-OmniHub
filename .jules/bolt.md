...
## 2026-06-25 - Extracted repeated array filtering inside hot loop stream block
**Learning:** Performing a chained `.filter().map().join()` sequence on an array of messages to extract system prompt text is expensive. Doing it twice—once before processing a proxy request and again inside the stream completion block—unnecessarily wastes CPU cycles and creates extra memory allocations in a high-throughput edge function.
**Action:** When data extraction is needed across multiple phases of a function (like pre-flight checking and post-flight caching), extract it to a higher-scoped constant at the start of the function and reuse that reference everywhere.
